...
- recursively do the same for all the embedded links
    ...
    - have a global list of links extracted. (should it be in a list or recorded in a file? --> a json file)
        - make sure the all the links in the global list are unique
        - when extracting the links, check against the global list to see if the link has been extracted before
